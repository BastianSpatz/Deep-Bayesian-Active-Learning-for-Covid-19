...
def convert_images_to_npy(args):
    ...
    for _, row in df.iterrows():
        path_to_volume_folder = path_to_folders + row["label"] + "/" + str(row["patient_id"]) + "/" + str(row["scan_id"])
        ...
        print("Patient: {} scan id: {}".format(row["patient_id"], row["scan_id"]), end="\r")
        if row["label"] == "CP":
            y = 0
        elif row["label"] == "NCP":
            y = 1
        ...
        try:
            ...
            np.save(args.save_path + "volumes/" + "vol_" + row["label"] + "_" + str(row["patient_id"]) + "_" + str(row["scan_id"]), np.array(volume))
            np.save(args.save_path + "labels/" + "label_" + row["label"] + "_" + str(row["patient_id"]) + "_" + str(row["scan_id"]), np.array(y))
