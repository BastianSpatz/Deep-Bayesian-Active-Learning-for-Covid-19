...
class ConvModule(LightningModule):
    ...
    def __init__(self, model, args):
        ...
        self.train_transform = transforms.Compose([#transforms.Resize((512//2, 512//2)),
        ...
        self.test_transform = transforms.Compose([#transforms.Resize((512//2, 512//2)),
                                                transforms.Normalize((0.5, ), (0.5, )),
                                            ])
        self.save_hyperparameters()
        self.dataset = CustomDataset(data_path=args.path_to_npy_data,
                             target_path=args.path_to_npy_targets)
        self.train_indices, self.val_indices, self.test_indices, _= get_active_learning_datasets(self.dataset, split=[0.1, 0.8, 0.1])
    ...
    def train_dataloader(self):
        ...
        train_datset = CustomFileDataset([self.dataset.data_paths[idx] for idx in list(self.train_indices)],
    ...
    def val_dataloader(self):
        test_datset = CustomFileDataset([self.dataset.data_paths[idx] for idx in list(self.test_indices)],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--learning_rate', default=9e-7, type=float, help='learning rate')
