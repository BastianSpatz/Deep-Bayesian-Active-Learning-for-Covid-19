    ...
    def __init__(
        ...
        **kwargs,
        ...
        if ndata_to_label is not None:
            warnings.warn(
                "`ndata_to_label` is deprecated, please use `query_size`.",
                DeprecationWarning,
            )
        ...
        if len(pool) > 0:
            ...
            if probs is not None and (
                ...
                if len(to_label) > 0:
                    ...
                    labelled_classes = [
                        self.dataset._dataset[idx][1].detach().cpu().numpy()
                        for idx in to_label[: self.query_size]
                    ]
                    print(
                        "Number of CP examples choosen: {}".format(
                            labelled_classes.count(0)
                        )
                    )
                    print(
                        "Number of NCP examples choosen: {}".format(
                            labelled_classes.count(1)
                        )
                    )
                    print(
                        "Number of Normal examples choosen: {}".format(
                            labelled_classes.count(2)
                        )
                    )
                    return True
        return False
